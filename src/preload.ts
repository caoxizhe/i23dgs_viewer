...
export async function runPreload(events: Events) {
  ...
  if (primaryUrl) {
    try {
      const fname = (() => { try { return new URL(primaryUrl, document.baseURI).pathname.split('/').pop(); } catch { return primaryUrl; } })();
      await events.invoke('import', [{ filename: fname, url: primaryUrl }]);
    } catch (e) { console.warn('[preload] primary import failed', e); }
  }
  let camerasJson: any[] | null = null;
  if (camerasUrl) {
    // 可选：尝试通过 import 让资源做缓存（JSON 分支在 v2 需要 contents，可能失败，失败也不影响后续 fetch）
    try {
      const fname = (() => { try { return new URL(camerasUrl, document.baseURI).pathname.split('/').pop(); } catch { return camerasUrl; } })();
