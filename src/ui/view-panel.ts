...
class ViewPanel extends Container {
    constructor(events: Events, tooltips: Tooltips, args = {}) {
        ...
        const remoteControlScaleRow = new Container({
            class: 'view-panel-row'
        });

        const remoteControlScaleLabel = new Label({
            text: localize('panel.view-options.remote-control-scale'),
            class: 'view-panel-row-label'
        });

        const remoteControlScaleSlider = new SliderInput({
            class: 'view-panel-row-slider',
            min: 0.1,
            max: 10,
            precision: 2,
            value: Number(localStorage.getItem('remote.scale') ?? '1') || 1
        });

        remoteControlScaleRow.append(remoteControlScaleLabel);
        remoteControlScaleRow.append(remoteControlScaleSlider);

        // camera frustum visibility
        const frustumRow = new Container({
            class: 'view-panel-row'
        });

        const frustumLabel = new Label({
            text: localize('panel.view-options.show-frustums'),
            class: 'view-panel-row-label'
        });

        const frustumToggle = new BooleanInput({
            type: 'toggle',
            class: 'view-panel-row-toggle',
            value: false
        });

        frustumRow.append(frustumLabel);
        frustumRow.append(frustumToggle);

        // camera rotation mode: yaw-only toggle
        const yawOnlyRow = new Container({
            class: 'view-panel-row'
        });

        const yawOnlyLabel = new Label({
            text: localize('panel.view-options.yaw-only'),
            class: 'view-panel-row-label'
        });

        const yawOnlyToggle = new BooleanInput({
            type: 'toggle',
            class: 'view-panel-row-toggle',
            value: true
        });

        yawOnlyRow.append(yawOnlyLabel);
        yawOnlyRow.append(yawOnlyToggle);
        ...
        this.append(remoteControlScaleRow);
        this.append(frustumRow);
        this.append(yawOnlyRow);
        ...
        events.on('remote.scale', (value: number) => {
            remoteControlScaleSlider.value = value;
        });

        remoteControlScaleSlider.on('change', (value: number) => {
            events.fire('remote.setScale', value);
        });

        // camera frustum visibility
        events.on('camera.frustumVisible', (value: boolean) => {
            frustumToggle.value = value;
        });

        frustumToggle.on('change', (value: boolean) => {
            events.fire('camera.setFrustumVisible', value);
        });

        // yaw-only toggle
        events.on('camera.yawOnly', (value: boolean) => {
            yawOnlyToggle.value = value;
        });

        yawOnlyToggle.on('change', (value: boolean) => {
            events.fire('camera.setYawOnly', value);
        });
