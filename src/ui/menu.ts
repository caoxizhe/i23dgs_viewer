...
class Menu extends Container {
    constructor(events: Events, args = {}) {
        ...
        const immersiveToggle = new Label({
            text: 'ON',
            id: 'menu-immersive-toggle',
            class: 'menu-immersive-toggle'
        });

        let immersive = false;
        const setImmersive = (value: boolean) => {
            immersive = !!value;
            document.body.classList.toggle('immersive-ui', immersive);
            immersiveToggle.class[!immersive ? 'add' : 'remove']('active');
            immersiveToggle.text = immersive ? 'OFF' : 'ON';
            events.fire('ui.immersive', immersive);
        };

        immersiveToggle.dom.addEventListener('click', (event: MouseEvent) => {
            event.stopPropagation();
        ...
        buttonsContainer.append(immersiveToggle);
        ...
        events.function('ui.immersive', () => immersive);
        events.on('ui.setImmersive', (value: boolean) => setImmersive(value));
        setImmersive(false);
    }
}
