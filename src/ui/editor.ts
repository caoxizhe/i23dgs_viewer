...
class EditorUI {
    ...
    constructor(events: Events) {
        ...
        const imageContainer = new ImageContainer(events);
        ...
        events.function('show.about', () => {
            return this.popup.show({
                type: 'info',
                header: 'About',
                message: `I23DGS Viewer v${version}`
            });
        });
        ...
        try {
            const raw = localStorage.getItem('camera.frustumsVisible');
            if (raw !== null) frustumsVisible = raw === 'true';
        } catch { /* noop */ }

        events.function('camera.frustumsVisible', () => {
            return frustumsVisible;
        });
        ...
        // persistent UI state: camera rotation mode ('horizontal' | 'free')
        let rotationMode = 'horizontal';
        try {
            const raw = localStorage.getItem('camera.rotationMode');
            if (raw === 'free' || raw === 'horizontal') rotationMode = raw;
        } catch { /* noop */ }

        events.function('camera.rotationMode', () => {
            return rotationMode;
        });
