...
import { registerRemoteControlEvents } from './remote-control';
...
const initShortcuts = (events: Events) => {
    ...
    shortcuts.register(['E', 'e'], { event: 'tool.eyedropperSelection', sticky: true, shift: true });
    shortcuts.register(['A', 'a'], { event: 'select.all', ctrl: true });
    shortcuts.register(['N', 'n'], { event: 'select.none' });
    ...
    shortcuts.register(['V', 'v'], { event: 'dataPanel.toggle' });
...
const main = async () => {
    ...
    // register image events before creating UI (ImageContainer needs them)
    registerImageEvents(events);
    ...
    registerRemoteControlEvents(events);
