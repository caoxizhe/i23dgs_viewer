...
class PointerController {
    ...
    constructor(camera: Camera, target: HTMLElement) {
        ...
        const orbit = (dx: number, dy: number) => {
            // use free-look rotation for mouse drag
        ...
        const keydown = (event: KeyboardEvent) => {
            // Only allow camera control when timeline is not playing
            if (camera.scene.events.invoke('timeline.playing')) {
                return;
            }
            const rawKey = event.key;
            const key = rawKey.length === 1 ? rawKey.toLowerCase() : rawKey;
            if (keys.hasOwnProperty(key) && event.target === document.body) {
                keys[key] = event.shiftKey ? 10 : (event.ctrlKey || event.metaKey || event.altKey ? 0.1 : 1);
            }
        };
        ...
        this.update = (deltaTime: number) => {
            const arrowx = keys.ArrowRight - keys.ArrowLeft;
            const arrowz = keys.ArrowDown - keys.ArrowUp;

            // 计算 WASD 移动（第一人称控制）
            const wasdX = keys.d - keys.a; // A左移，D右移
            const wasdForward = keys.s - keys.w; // W前进，S后退
            const wasdUp = keys.e - keys.q; // Q下降，E上升

            // 合并箭头键和WASD输入
            const x = arrowx + wasdX; // 横向
            const forward = arrowz + wasdForward; // 前后（传入为第二参数）
            const up = wasdUp; // 垂直（Z 轴向上）

            if (x || forward || up) {
                const factor = deltaTime * camera.flySpeed;
                // 注意：Camera.moveByInput 的参数顺序为 (strafe, forward, vertical, factor)
                camera.moveByInput(x, forward, up, factor);
            }
        };
