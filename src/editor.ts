...
const registerEditorEvents = (events: Events, editHistory: EditHistory, scene: Scene) => {
    ...
    const setFrustumVisible = (value: boolean) => {
        const v = !!value;
        if (v !== frustumVisible) {
            frustumVisible = v;
            events.fire('camera.frustumVisible', frustumVisible);
            scene.forceRender = true;
        }
    };

    events.function('camera.frustumVisible', () => frustumVisible);

    events.on('camera.setFrustumVisible', (value: boolean) => {
        setFrustumVisible(value);
    });

    events.on('camera.toggleFrustumVisible', () => {
        setFrustumVisible(!frustumVisible);
    });

    // ensure new camera poses refresh debug rendering when visible
    events.on('camera.posesLoaded', () => {
        if (frustumVisible) {
            scene.forceRender = true;
        }
    });

    // camera yaw-only (horizontal-only rotation) toggle

    let yawOnly = true;
    scene.camera.yawOnly = yawOnly;

    const setYawOnly = (value: boolean) => {
        const v = !!value;
        if (v !== yawOnly) {
            yawOnly = v;
            scene.camera.yawOnly = v;
            events.fire('camera.yawOnly', yawOnly);
        }
    };

    events.function('camera.yawOnly', () => yawOnly);

    events.on('camera.setYawOnly', (value: boolean) => {
        setYawOnly(value);
    });
    ...
    // expose camera entity for debug overlays (caption box)
    events.function('camera.entity', () => scene.camera.entity);

    events.on('camera.setPose', (pose: { position: Vec3, target: Vec3 }, speed = 1) => {
        scene.camera.setPose(pose.position, pose.target, speed);
    });

    events.on('camera.remoteTranslate', (payload: { dx: number, dy: number, dz: number }) => {
        remoteDelta.set(payload.dx, payload.dy, payload.dz);
        scene.camera.moveByWorldDelta(remoteDelta);
        scene.forceRender = true;
    });

    // bridge: camera.setView (align render view to original image)
    events.on('camera.setView', (payload: any) => {
        if (!payload || !payload.position || !payload.rotation) return;
        const pos: Vec3 = payload.position;
        const rot: number[] | number[][] = payload.rotation;
        const fx: number | undefined = payload.fx;
        const fy: number | undefined = payload.fy;
        const speed: number = typeof payload.speed === 'number' ? payload.speed : 1;
        scene.camera.setView(pos, rot, fx, fy, speed);
    });
    ...
    events.function('docSerialize.view', () => {
        ...
        return {
            ...
            yawOnly: events.invoke('camera.yawOnly'),
            frustumVisible: events.invoke('camera.frustumVisible')
        };
    });
    ...
    events.function('docDeserialize.view', (docView: any) => {
        ...
        events.fire('camera.setYawOnly', docView.yawOnly ?? true);
        events.fire('camera.setFrustumVisible', docView.frustumVisible ?? false);
    });

    // init yawOnly default broadcast
    events.fire('camera.yawOnly', yawOnly);
    // init frustum visibility (default off)
    events.fire('camera.frustumVisible', frustumVisible);
};
